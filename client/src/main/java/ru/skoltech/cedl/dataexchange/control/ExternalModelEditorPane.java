...
public class ExternalModelEditorPane implements Initializable {
        ...
        if (externalModelFile != null) {
            ...
            if (externalModelFile.isFile() && ExternalModelAccessorFactory.hasEvaluator(fileName)) {
                try {
                    ...
                    Platform.runLater(ExternalModelEditorPane.this::updateView);
                    Dialogues.showWarning("The file is now under CEDESK version control.", "The file has been imported into the repository. Further modifications on the local copy will not be reflected in the system model!");
    ...
    private void renderExternalModelView(ExternalModel externalModel) {
        HBox editorHBox = guiService.createControl(Controls.EXTERNAL_MODEL_CONTROL, externalModel);
        ...
        removeButton.setOnAction(ExternalModelEditorPane.this::deleteExternalModel);
        ...
        exchangeButton.setOnAction(ExternalModelEditorPane.this::exchangeExternalModel);
        exchangeButton.setMinWidth(28);
        HBox extModRow = new HBox(6, editorHBox, removeButton, exchangeButton);
